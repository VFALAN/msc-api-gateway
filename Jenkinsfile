        ...
        stage('Checkout Code') {
            steps {
                git branch: 'development', // Replace 'main' with your branch name
                    ...
                    url: 'https://github.com/VFALAN/msc-api-gateway.git' // Replace with your Git repository URL
            }
        }
        stage('Build Project') {
            steps {
            bat 'set LOKI_URL=http://localhost:3100/loki/api/push'
            ...
            steps {
                script {
                    docker.withRegistry('https://registry.hub.docker.com','dcoker-credentials'){
                        def dockerImageBuild = docker.build("vifa951002/msc-api-gateway")
